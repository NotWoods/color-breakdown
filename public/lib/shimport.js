var __shimport__ = (function(exports) {
    ...
    function importDecl(str, start, end, specifiers, source) {
        var name =
            get_alias(specifiers, '*') || get_alias(specifiers, 'default');
        return {
            start: start,
            end: end,
            source: source,
    ...
    function exportDefaultDeclaration(str, start, end) {
        ...
        var match = /^\s*(?:(class)(\s+extends|\s*{)|(function)\s*\()/.exec(
            str.slice(end),
        );
        if (match) {
    ...
    function transform(source, id) {
        ...
        for (var i = 0; i < ranges.length; i += 1) {
            var range = ranges[i];
            transformed +=
                source.slice(c, range.start) + range.toString(nameBySource);
            c = range.end;
        }
    ...
    function load(url) {
        return (
            ...
            (promises[url] = fetch(url)
                ...
                .then(function(text) {
                    return eval(transform(text, url));
                }))
        );
    }
